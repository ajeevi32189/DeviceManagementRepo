...
namespace DeviceManagement.Data
{
    public class DBContext : DbContext
    {
        ...
        // ── Import / Export ────────────────────────────────────────
        public DbSet<ImportFieldMapping> ImportFieldMappings { get; set; }

        // ── Model Protocol Profiles & Points ─────────────────────────
        public DbSet<ModelProtocolProfile> ModelProtocolProfiles => Set<ModelProtocolProfile>();
        public DbSet<ModelProtocolPoint> ModelProtocolPoints => Set<ModelProtocolPoint>();
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<ModelProtocolProfile>(e =>
            {
                e.ToTable("modelprotocolprofiles");
                e.HasKey(x => x.Id);

                e.Property(x => x.Protocol).HasMaxLength(32).IsRequired();
                e.Property(x => x.WordOrder).HasMaxLength(8).HasDefaultValue("BIG");
                e.Property(x => x.ByteOrder).HasMaxLength(8).HasDefaultValue("BIG");
                e.Property(x => x.AddressBase).HasDefaultValue((byte)0);
                e.Property(x => x.PollPeriodMs).HasDefaultValue(10000);
                e.Property(x => x.Source).HasMaxLength(512);
                e.Property(x => x.VerifiedBy).HasMaxLength(255);
                e.Property(x => x.IsDeleted).HasDefaultValue(false);

                // NOTE: NOT marked .IsUnique() — soft-delete was chosen, and MySQL has no filtered
                // unique index, so a deleted row would permanently block re-creating the same key.
                // Uniqueness of (ModelSpecificationId, Protocol) among IsDeleted == false rows
                // must be enforced in the service layer before insert.
                e.HasIndex(x => new { x.ModelSpecificationId, x.Protocol })
                 .HasDatabaseName("ix_model_protocol");

                e.HasOne(x => x.ModelSpecification)
                 .WithMany()
                 .HasForeignKey(x => x.ModelSpecificationId)
                 .OnDelete(DeleteBehavior.Restrict);
            });

            modelBuilder.Entity<ModelProtocolPoint>(e =>
            {
                e.ToTable("modelprotocolpoints");
                e.HasKey(x => x.Id);

                e.Property(x => x.PointKey).HasMaxLength(128).IsRequired();
                ...
                e.Property(x => x.Confidence).HasMaxLength(16).HasDefaultValue("confirmed");
                e.Property(x => x.RegisterCount).HasDefaultValue(1);
                e.Property(x => x.Multiplier).HasDefaultValue(1d);
                e.Property(x => x.IsEnabled).HasDefaultValue(true);
                e.Property(x => x.IsDeleted).HasDefaultValue(false);

                // Same reasoning as above — not unique at the DB level because of soft-delete.
                e.HasIndex(x => new { x.ProfileId, x.PointKey })
