...
namespace DeviceManagementOnly.Services
{
    ...
    public class DeviceDetailService : IDeviceDetailService
    {
        ...
        public DeviceDetailService(DBContext db, DeviceManagementOnly.Common.ImportExport.IImportService importService)
        {
            _db = db;
        ...
        {
            if (file == null || file.Length == 0) throw new Exception("Please upload a valid Excel/CSV file.");

            var response = new DeviceDetailImportResponse();

            // ── Dynamic, header-based read (works for BOTH .xlsx and .csv) ──
            // Known backend columns for this import. A saved mapping (FieldMappingController)
            // is tried first; otherwise headers are auto-matched to these names
            // (case/spacing-insensitive). Anything left over is "unmatched" and
            // gets written to a downloadable .txt instead of being silently dropped.
            var knownColumns = new[]
            {
                nameof(DeviceDetailImportRow.CategoryName), nameof(DeviceDetailImportRow.DeviceTypeName),
                nameof(DeviceDetailImportRow.ModelName), nameof(DeviceDetailImportRow.ModelNumber),
                nameof(DeviceDetailImportRow.DeviceShortName), nameof(DeviceDetailImportRow.DeviceLongName),
                nameof(DeviceDetailImportRow.IMEI), nameof(DeviceDetailImportRow.MACAddress),
                nameof(DeviceDetailImportRow.TagNumber), nameof(DeviceDetailImportRow.SerialNumber),
                nameof(DeviceDetailImportRow.PurchaseDate), nameof(DeviceDetailImportRow.WarrantyExpiry),
                nameof(DeviceDetailImportRow.PurchaseCost), nameof(DeviceDetailImportRow.IPAddress),
                nameof(DeviceDetailImportRow.SIMNumber), nameof(DeviceDetailImportRow.Remarks)
            };

            var (sheet, headerToField, unmatchedColumns) =
                await _importService.ResolveMappingAsync(file, "DeviceDetail", null, knownColumns);

            var rows = new List<DeviceDetailImportRow>(sheet.Rows.Count);
            ...
            for (int i = 0; i < sheet.Rows.Count; i++)
            {
                ...
                var importRow = new DeviceDetailImportRow();
                DeviceManagementOnly.Common.ImportExport.RowMapper.PopulateFromRow(importRow, sheet.Headers, raw, headerToField);
                rows.Add(importRow);
            ...
            if (unmatchedRows.Count > 0)
            {
                ...
                response.UnmatchedFileUrl = await _importService.SaveUnmatchedFileAsync(
                    "DeviceDetail", unmatchedColumns, unmatchedRows, file.FileName);
            }
