...
namespace DeviceManagementOnly.Services
{
    ...
    public class DeviceCategoryService : IDeviceCategoryService
    {
        ...
        public DeviceCategoryService(DBContext db, DeviceManagementOnly.Common.ImportExport.IImportService importService)
        {
            _db = db;
        ...
        public async Task<DeviceCategoryImportResponse> ImportDeviceCategoryExcelAsync(IFormFile file)
        {
            if (file == null || file.Length == 0)
                throw new Exception("Please upload a valid Excel (.xlsx) or CSV (.csv) file.");

            var imported = new List<string>();
            var skipped = new List<string>();
            var toAdd = new List<DeviceCategory>();

            // ── Dynamic, header-based read (works for BOTH .xlsx and .csv) ──
            var knownColumns = new[] { "Name", "CategoryName", "Remarks", "CategoryDescription" };
            var (sheet, headerToField, unmatchedColumns) =
                await _importService.ResolveMappingAsync(file, "DeviceCategory", null, knownColumns);
            ...
            for (int i = 0; i < sheet.Rows.Count; i++)
            {
                ...
                string? Get(params string[] possibleHeaders)
                {
                    foreach (var h in possibleHeaders)
                        if (raw.TryGetValue(h, out var v) && !string.IsNullOrWhiteSpace(v)) return v.Trim();
                    return null;
                }

                var name = Get("Name", "CategoryName");
                var remarks = Get("Remarks", "CategoryDescription");
            ...
            var response = new DeviceCategoryImportResponse
            {
            ...
            if (unmatchedRows.Count > 0)
            {
                ...
                response.UnmatchedColumns = unmatchedColumns;
                response.UnmatchedFileUrl = await _importService.SaveUnmatchedFileAsync(
                    "DeviceCategory", unmatchedColumns, unmatchedRows, file.FileName);
            }
    ...
    public class DeviceTypeService : IDeviceTypeService
    {
        private readonly DBContext _db;
        private readonly DeviceManagementOnly.Common.ImportExport.IImportService _importService;
        public DeviceTypeService(DBContext db, DeviceManagementOnly.Common.ImportExport.IImportService importService)
        {
            _db = db;
            _importService = importService;
        }
        ...
        public async Task<PagedResult<DeviceTypeResponseDto>> GetAllDeviceTypeAsync(PaginationParams p)
        {
            ...
            if (!string.IsNullOrWhiteSpace(p.Search))
            {
                ...
                query = query.Where(x =>
                    ...
                    (x.Remarks != null && x.Remarks.ToLower().Contains(search)) ||
                    (x.Icon != null && x.Icon.ToLower().Contains(search)) ||
                    (x.IconColor != null && x.IconColor.ToLower().Contains(search)) ||
        ...
        public async Task<DeviceTypeImportResponse> ImportDeviceTypeExcelAsync(IFormFile file)
        {
            ...
            // ── Dynamic, header-based read (works for BOTH .xlsx and .csv) ──
            var knownColumns = new[] { "Name", "CategoryName", "Protocol", "SupportedFeature", "Remarks" };
            var (sheet, headerToField, unmatchedColumns) =
                await _importService.ResolveMappingAsync(file, "DeviceType", null, knownColumns);

            var response = new DeviceTypeImportResponse();
            var unmatchedRows = new List<DeviceManagementOnly.Common.ImportExport.UnmatchedDataWriter.UnmatchedRow>();
            ...
            for (int i = 0; i < sheet.Rows.Count; i++)
            {
                ...
                string? Get(string col) => headerToField.ContainsKey(col) || sheet.Headers.Contains(col)
                    ? (raw.TryGetValue(col, out var v) ? v : null) : null;

                var typeName = Get("Name")?.Trim();
                var categoryName = Get("CategoryName")?.Trim();
                var protocol = Get("Protocol")?.Trim();
                var supportedFeature = Get("SupportedFeature")?.Trim();
                var remarks = Get("Remarks")?.Trim();
            ...
            response.TotalImported = imported.Count;
            response.TotalSkipped = skipped.Count;
            response.ImportedDeviceTypes = imported;
            response.SkippedDeviceTypes = skipped;
            ...
            if (unmatchedRows.Count > 0)
            {
                ...
                response.UnmatchedColumns = unmatchedColumns;
                response.UnmatchedFileUrl = await _importService.SaveUnmatchedFileAsync(
                    "DeviceType", unmatchedColumns, unmatchedRows, file.FileName);
            }
    ...
    public interface IModelParameterService
    {
        ...
        Task<List<ModelParameterResponseDto>> GetAllModelParameterAsync();
    ...
    public class ModelParameterService : IModelParameterService
    {
        ...
        public async Task<List<ModelParameterResponseDto>> GetAllModelParameterAsync()
        {
            var list = await _db.ModelParameters.AsNoTracking()
                .Where(x => !x.IsDeleted).OrderBy(x => x.ParameterName).ToListAsync();
            var result = new List<ModelParameterResponseDto>();
            foreach (var p in list) result.Add(await MapWithCategory(p));
            return result;
        }
