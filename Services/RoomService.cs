...
namespace DeviceManagementOnly.Services
{
    ...
    public interface IRoomService
    {
        ...
        // ── DXF floor-plan import/export ──
        Task<DxfImportResultDto> ImportDxfAsync(Guid floorId, IFormFile file, string? updatedBy);
        Task<byte[]> ExportDxfAsync(Guid floorId);
    }
    ...
    public class RoomService : IRoomService
    {
        ...
        // ══════════════════════════════════════════════════════════
        //  DXF FLOOR-PLAN IMPORT / EXPORT
        //  Convention: one closed LWPOLYLINE rectangle per room, the
        //  polyline's LAYER name = Room name. See DxfIo for details.
        // ══════════════════════════════════════════════════════════

        public async Task<DxfImportResultDto> ImportDxfAsync(Guid floorId, IFormFile file, string? updatedBy)
        {
            if (file == null || file.Length == 0) throw new InvalidOperationException("Please upload a .dxf file.");

            var floor = await _db.Floors.AsNoTracking().FirstOrDefaultAsync(f => f.Id == floorId)
                ?? throw new InvalidOperationException($"Floor {floorId} not found.");

            List<DeviceManagementOnly.Common.ImportExport.DxfIo.DxfRoom> dxfRooms;
            using (var stream = file.OpenReadStream())
            {
                dxfRooms = DeviceManagementOnly.Common.ImportExport.DxfIo.ParseRooms(stream);
            }

            var result = new DxfImportResultDto { TotalRoomsInFile = dxfRooms.Count };
            if (dxfRooms.Count == 0)
            {
                result.Message = "No room rectangles (LWPOLYLINE) found in the DXF file.";
                return result;
            }

            var existingRooms = await _db.Rooms.Where(r => r.FloorId == floorId).ToListAsync();
            var existingByName = existingRooms.ToDictionary(r => r.Name, StringComparer.OrdinalIgnoreCase);

            foreach (var dr in dxfRooms)
            {
                try
                {
                    if (existingByName.TryGetValue(dr.Name, out var existing))
                    {
                        existing.PositionX = dr.PositionX;
                        existing.PositionY = dr.PositionY;
                        existing.Width = dr.Width;
                        existing.Length = dr.Length;
                    ...
                    else
                    {
                        var room = new Room
                        {
                            FloorId = floorId,
                            Name = dr.Name,
                            PositionX = dr.PositionX,
                            PositionY = dr.PositionY,
                            Width = dr.Width,
                            Length = dr.Length,
                            RotationAngle = 0,
                            IsActive = true,
                        ...
                        _db.Rooms.Add(room);
                        existingByName[dr.Name] = room;
                        result.Created++;
                    }
                }
                catch (Exception ex)
                {
                    result.Errors.Add($"{dr.Name}: {ex.Message}");
                }
            }

            await _db.SaveChangesAsync();

            result.Message = $"DXF import complete: {result.Created} room(s) created, {result.Updated} updated" +
                              (result.Errors.Count > 0 ? $", {result.Errors.Count} failed." : ".");

            // Notify any open 3D room-layout viewers for this floor.
            await _hub.Clients.Group($"floor-{floorId}").SendAsync("FloorLayoutChanged", new { floorId });

            return result;
        }

        public async Task<byte[]> ExportDxfAsync(Guid floorId)
        {
            var floor = await _db.Floors.AsNoTracking().FirstOrDefaultAsync(f => f.Id == floorId)
                ?? throw new InvalidOperationException($"Floor {floorId} not found.");
            ...
            var dxfRooms = rooms
                .Where(r => r.PositionX.HasValue && r.PositionY.HasValue && r.Width.HasValue && r.Length.HasValue)
                .Select(r => new DeviceManagementOnly.Common.ImportExport.DxfIo.DxfRoom
                {
                    Name = r.Name,
            ...
            return DeviceManagementOnly.Common.ImportExport.DxfIo.BuildDxf(floor.Name, dxfRooms);
        }
