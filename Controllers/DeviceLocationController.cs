...
namespace DeviceManagementOnly.Controllers
{
    ...
    public class DeviceLocationController : ControllerBase
    {
        ...
        // GET: api/DeviceLocation/export?format=excel|csv|pdf
        [HttpGet("export")]
        public async Task<IActionResult> Export([FromQuery] string format, [FromQuery] PaginationParams p)
        {
            ...
            var result = await _service.GetAllDeviceLocationAsync(p);
            var file = await _exportService.ExportAsync(result.Data, exportFormat, "DeviceLocations");
            return File(file.Content, file.ContentType, file.FileName);
        }

        // POST: api/DeviceLocation/import — Excel/CSV, dynamic mapping, unmatched-.txt
        // NOTE: DeviceDetailId column must contain a valid GUID. Photo upload isn't
        // supported via bulk import (that's a per-record UI action).
        [HttpPost("import")]
        public async Task<IActionResult> Import(IFormFile file)
        {
            ...
            var result = await _importService.ImportAsync<DeviceLocationCreateDto>(
                file, "DeviceLocation", null,
                async (dto, rowNumber) => await _service.CreateDeviceLocationAsync(dto, UploadBasePath));
